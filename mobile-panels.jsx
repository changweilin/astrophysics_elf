...
function MBodyEditor({ sim, force, role }) {
  ...
  // stage drivers (age, metallicity, spin, magnetic field). KNSim.syncStellar
  ...
  // Stage drivers — R★/T★ are re-derived from these by KNSim.syncStellar.
  function setAge(v) {
    if (isCentral) sim.params.age = v; else if (bin) bin.age2 = v;
    force();
  }
  ...
  function switchCategory(cat) {
    ...
    if (isCentral) {
      ...
      if (saved) {
        ...
        sim.params.age = 0; sim.params.Z = 0.5;
        if (newType !== 'bh') {
  ...
  // Galaxy / cluster mode: the mass slider reads/writes the structure's TOTAL mass
  // (BH + stars + clouds + DM); the hole is the bhFrac share (mirrors panel-left).
  const companionStructure0 = (bin && bin.smbhStructure)
    || ((sim.disc2 && sim.disc2.enabled) ? 'galaxy' : 'smbh');
  const activeSMBHKey = isCentral ? (sim.smbhStructure || 'smbh') : companionStructure0;
  const structTotalMode = regime === 'supermassive'
  ...
  return (
    <>
      ...
      {((smbhStructures && (sim.smbhStructure === 'galaxy' || sim.smbhStructure === 'cluster')) || (isCentral && inOpenCluster)) && (
        ...
        {/* Mass budget (mirrors panel-left): total + component shares, dark matter
            as the remainder — KNSim.structureShares() is the single source. */}
        {(() => {
        ...
        {sim.smbhStructure === 'galaxy' && smbhStructures && (
          <React.Fragment>
          {/* Dedicated AGN switch (mirrors panel-left): the galaxy's active-nucleus
              state via setDiscEnabled; merger gas inflow can also ignite it. */}
          <button className={`m-disc-toggle ${sim.disc && sim.disc.enabled ? 'on' : ''}`}
          ...
          {/* Post-G×G-merger morphology (violent relaxation → elliptical). */}
          ...
          <MParam sym="f" name={tr('Molecular clouds (of total mass)', '分子雲質量（占總質量）')} unit=""
          ...
          <MParam sym="f₍BH₎" name={tr('Black hole mass share', '黑洞質量比')} unit=""
      ...
      {((companionStructures && (companionStructure === 'galaxy' || companionStructure === 'cluster')) || (!isCentral && inOpenCluster)) && (
        ...
        {/* Mass budget (mirrors the central block above). */}
        {(() => {
        ...
        {companionStructure === 'galaxy' && companionStructures && (
          <React.Fragment>
          <button className={`m-disc-toggle ${sim.disc2 && sim.disc2.enabled ? 'on' : ''}`}
      ...
      <MParam sym={isCentral ? 'M' : 'M₂'}
              name={structTotalMode ? tr('Total mass', '總質量') : tr('Mass', '質量')}
              val={A.Msun / fBH} unit={A.massUnit}
      ...
      <div className="stellar-sub">
        <div className="sub-head">
          ...
          <span className="hint">{isBH ? tr('sealed under r₊', '封閉於 r₊ 之內') : (collapseHint ? tr('◆ will collapse', '◆ 將塌縮') : surfaceDriver)}</span>
        </div>

        {/* Stage drivers — the only knobs; R★/T★/colour are derived from them. */}
        {A.type === 'ms' && (
          <MParam sym={isCentral ? 'τ' : 'τ₂'} name={tr('Age (of main-seq. life)', '年齡（佔主序壽命）')}
                  val={A.age} unit=""
        ...
        {A.type === 'giant' && (
          <MParam sym={isCentral ? 'Z' : 'Z₂'} name={tr('Metallicity (heavy elements)', '金屬量（重元素）')}
                  val={A.Z} unit=""
                  min={0} max={1} step={0.01}
        ...
        {(A.type === 'wd' || A.type === 'ns') && (
          <div className="sub-note" style={{ fontSize: '0.78em', opacity: 0.7, margin: '2px 0 8px' }}>
            {A.type === 'wd'
              ? tr('R★ shrinks with mass; spin enlarges it, mass + field heat it.',
                   'R★ 隨質量縮小；自旋使其變大，質量與磁場使其變熱。')
              : tr('R★ ≈ const; spin flattens it; field (magnetar) sets the temperature.',
          ...
          {M_PRESETS.map((pr, i) => (
            <button key={i}
              onClick={() => {
                ...
                sim.params.age = 0; sim.params.Z = 0.5;     // ms/giant: ZAMS / solar
                // ms & giant surfaces derive from mass; wd/ns honour the preset value.
